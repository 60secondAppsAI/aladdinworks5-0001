					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updatePowerSupply()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2PowerSupplys = true">Add</base-button>
								              <modal :show.sync="modal2PowerSupplys">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Power Supply Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PowerSupplyId" type="text" placeholder="Enter PowerSupplyId" v-model="powerSupplyDetails.powerSupplyId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Capacity</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Capacity" type="text" placeholder="Enter Capacity" v-model="powerSupplyDetails.capacity"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Status</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Status" type="text" placeholder="Enter Status" v-model="powerSupplyDetails.status"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updatePowerSupply() {
      ...
      const jsonData = JSON.stringify(this.powerSupplyDetails);
      const res = await PowerSupplyService.update(jsonData);
...
//        this.getAllPowerSupplys();
      }
    },


    async getPowerSupplyDetails() {
      try {
			let response = await PowerSupplyService.get(this.powerSupplyId);
			this.powerSupplyDetails = response.data;
        ...
        console.error('Error fetching powerSupply details:', error);
      }
    },
