					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updatePowerStrip()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2PowerStrips = true">Add</base-button>
								              <modal :show.sync="modal2PowerStrips">
								  ...
								  <base-input label="Sockets" type="text" placeholder="Enter Sockets" v-model="powerStripToAdd.sockets"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Power Strip Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PowerStripId" type="text" placeholder="Enter PowerStripId" v-model="powerStripDetails.powerStripId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Sockets</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Sockets" type="text" placeholder="Enter Sockets" v-model="powerStripDetails.sockets"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      powerStripToAdd: {},
      modal2PowerStrips: false,
      powerStripDetails: null,
  ...
  methods: {
    ...
    async updatePowerStrip() {

      const jsonData = JSON.stringify(this.powerStripDetails);
      const res = await PowerStripService.update(jsonData);
...
//        this.getAllPowerStrips();
    ...
    async getPowerStripDetails() {
      try {
			let response = await PowerStripService.get(this.powerStripId);
			this.powerStripDetails = response.data;
