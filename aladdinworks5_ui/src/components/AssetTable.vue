        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAssets = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAssets">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Asset</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AssetId" type="text" placeholder="Enter AssetId" v-model="assetToAdd.assetId"></base-input>
  <base-input label="AssetType" type="text" placeholder="Enter AssetType" v-model="assetToAdd.assetType"></base-input>
  <base-input label="PurchaseDate" type="text" placeholder="Enter PurchaseDate" v-model="assetToAdd.purchaseDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="assets" :row-key="record => record.AssetId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AssetPictureView :assets="assets" />
          </div>
...
import AssetPictureView from './AssetPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'assetId',
          visible: true,
          scopedSlots: { customRender: 'assetId' },
          sorter: true
      ...
      sortBy: 'assetId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAssetsTable() {
      ...
      for (let i = 0; i < this.assets.length; i++) {
        assetsTableData.push({
          ...
          year: this.assets[i].year,
          date: this.assets[i].date,
          competitionId: this.assets[i].competitionId,
          assetId: this.assets[i].assetId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-assets',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAssetsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.assetToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.assetToAdd);
      console.log(jsonData);
      
      const res = await AssetService.addAsset(jsonData);
