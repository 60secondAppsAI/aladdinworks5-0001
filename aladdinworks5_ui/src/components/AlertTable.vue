        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAlerts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAlerts">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Alert</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AlertId" type="text" placeholder="Enter AlertId" v-model="alertToAdd.alertId"></base-input>
  <base-input label="Severity" type="text" placeholder="Enter Severity" v-model="alertToAdd.severity"></base-input>
  <base-input label="Message" type="text" placeholder="Enter Message" v-model="alertToAdd.message"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="alerts" :row-key="record => record.AlertId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AlertPictureView :alerts="alerts" />
          </div>
...
import AlertPictureView from './AlertPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'alertId',
          visible: true,
          scopedSlots: { customRender: 'alertId' },
          sorter: true
          //sorter: (a, b) => a.alertId - b.alertId,
      ...
      sortBy: 'alertId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAlertsTable() {
      ...
      for (let i = 0; i < this.alerts.length; i++) {
        alertsTableData.push({
          id: i,
          alertId: this.alerts[i].alertId,
          year: this.alerts[i].year,
          date: this.alerts[i].date,
          competitionId: this.alerts[i].competitionId,
          alertId: this.alerts[i].alertId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-alerts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAlertsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.alertToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.alertToAdd);
      console.log(jsonData);
      
      const res = await AlertService.addAlert(jsonData);
